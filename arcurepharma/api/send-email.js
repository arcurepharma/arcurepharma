...
// --- Method 1: Send Contact Email ---
const sendContactEmail = async (data, res) => {
  const { name, email, subject, message } = data;
  const supportEmail = process.env.SUPPORT_EMAIL;

  // Create transport specific to Support Email
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 20px; border: 1px solid #e1e1e1; border-top: none; border-radius: 0 0 8px 8px;">
            <p>Hi ${name.split(' ')[0]},</p>
            <p>Thanks for getting in touch! We have received your message regarding "<strong>${subject}</strong>".</p>
            ...
            <br/>
            <p>Best Regards,<br/><strong>ArcurePharma Team</strong></p>
          </div>
        </div>
      `,
    });
...
const sendOrderEmail = async (data, res) => {
  ...
  // Create transport specific to Order Email
...
module.exports = async (req, res) => {
  ...
  if (type === "contact") {
    ...
    return await sendOrderEmail(data, res); // Default to order email
  }
};
